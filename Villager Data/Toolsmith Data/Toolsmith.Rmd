...
```{r}
fisherman_og <- read_csv("toolsmith_og.csv")
names(fisherman_og)
```
...
fisherman2.1 <- fisherman2 %>% 
  mutate(bedrock_edition_slot = case_when(villager_xp == "1" ~ "2", 
                           villager_xp == "10" & trades_in_stock == "3" ~ "6",
                           villager_xp == "15" ~ "8",
...
```{r}
write.csv(cleaner_data1.1, "toolsmith_clean_2.csv", row.names = FALSE)
toolsmith <- read_csv("toolsmith_clean_2.csv")
```
