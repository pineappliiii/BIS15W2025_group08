...
```{r}
fisherman_og <- read_csv("mason_og.csv")
names(fisherman_og)
```
...
fisherman2.1 <- fisherman2 %>% 
  mutate(bedrock_edition_slot = case_when(bedrock_edition_probability == "33%" ~ "5", 
                           bedrock_edition_probability == "25%" ~ "6",
                           item_given == "Any color Glazed Terracotta[t 1]" ~ "8",
                           item_given == "Block of Quartz" ~ "9",
...
fisherman2.8 <- fisherman2.4 %>% 
  ...
  mutate(num_item_given = as.integer(num_item_given))
fisherman2.8
```
...
```{r}
write.csv(cleaner_data1.1, "mason_clean_2.csv", row.names = FALSE)
mason <- read_csv("mason_clean_2.csv")
```
